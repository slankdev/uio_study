...
int main()
{
  intfd uio_fd("/dev/uio0", O_RDONLY);
  intfd configfd("/sys/class/uio/uio0/device/config", O_RDWR);
  printf("configfd: %d \n", configfd.get_fd());
  ...
  if (!(vendor_id==INTEL && device_id==IXGBE_X540))
    throw exception("device is not X540!!!");
  ...
  intfd fd("/sys/class/uio/uio0/device/resource0", O_RDWR);
